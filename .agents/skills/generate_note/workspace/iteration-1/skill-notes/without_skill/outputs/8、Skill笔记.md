...
在 `Claude Code` 中，**Skill（技能）** 是一种通过 `SKILL.md` 文件定义的可复用能力单元，用于扩展 Claude 能做的事情。当你反复在对话中粘贴相同的操作手册、检查清单或多步骤流程时，就应该考虑将其封装为一个 Skill。

Skill 与 `CLAUDE.md` 的核心区别在于加载时机：
- `CLAUDE.md` 的内容会在每次会话启动时自动加载到上下文中
- Skill 的主体内容**只在被使用时才加载**，因此长篇参考资料在未被调用前几乎不消耗上下文成本
...
**建议**：保持 `SKILL.md` 在 500 行以内。将详细参考资料移到独立文件，在 `SKILL.md` 中引用它们，让 Claude 知道每个文件的内容和加载时机。
...
如果 Skill 在首次响应后似乎停止影响行为，通常内容仍在上下文中，但模型选择了其他工具。可通过强化 `description` 和指令，或使用 [hooks](https://code.claude.com/docs/en/hooks) 确定性强制执行行为。
...
注意：设置了 `disable-model-invocation: true` 的 Skill 不能预加载到子代理中，因为预加载使用的是 Claude Code 可调用的 Skill 集合。

---

# 十、Skill 与 CLAUDE.md 的对比与选择
